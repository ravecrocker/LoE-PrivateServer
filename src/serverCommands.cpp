...
void App::sendCmdLine()
{
    ...
    if (str.startsWith("help"))
    {
...
#endif
        logMessage("stop");
        logMessage(QObject::tr("%1 Stops the game server").arg(indent));
        ...
        logMessage("sync");
        logMessage(QObject::tr("%1 Do a manual UDP sync").arg(indent));
        ...
        logMessage(QObject::tr("%1 Instantiate the given player with given values").arg(indent));
        logMessage("beginDialog");
        logMessage(QObject::tr("%1 Sends beginDialog to the selected player").arg(indent));
        logMessage("endDialog");
        logMessage(QObject::tr("%1 Sends endDialog to the selected player").arg(indent));
        logMessage("setDialogMsg <dialogue> <npcname>");
        logMessage(QObject::tr("%1 Sets dialogue message").arg(indent));
        logMessage("setDialogOptions <arguments separated by spaces>");
        logMessage(QObject::tr("%1 Sets the dialogue options, given args").arg(indent));
        ...
        logMessage(QObject::tr("%1 Sends an error to the client, given <data>").arg(indent));
